...
public:
    Path_Cal();
    bool calculate_path(const Struct_Algo::DroneData &drone_data, const std::vector<Struct_Algo::Coordinate> &points);
...
private:
    ...
    void solve_vrp(const std::vector<std::vector<int64_t>>& dist_matrix,
                   const std::vector<Struct_Algo::Coordinate>& pos_targets,
                   int num_drones);
};